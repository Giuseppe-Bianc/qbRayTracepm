...
bool qbRT::Cylinder::TestIntersection(const qbRT::Ray &castRay, qbVector<double> &intPoint, qbVector<double> &localNormal,
    ...
    if(numSQRT > 0.0) {
        ...
        if((t.at(0) > 0.0) && (std::fabs(poi.at(0).GetElement(2)) < 1.0)) {
            t1Valid = true;
        } else {
            t1Valid = false;
            t.at(0) = 100e6;
        ...
        if((t.at(1) > 0.0) && (std::fabs(poi.at(1).GetElement(2)) < 1.0)) {
            t2Valid = true;
        } else {
            t2Valid = false;
            t.at(1) = 100e6;
    ...
    if(CloseEnough(v.GetElement(2), 0.0)) {
        ...
        if((t.at(3) > 0.0) &&
           (std::sqrt(std::pow(poi.at(3).GetElement(0), 2.0) + std::pow(poi.at(3).GetElement(1), 2.0)) < 1.0)) {
            t4Valid = true;
        } else {
            t4Valid = false;
            t.at(3) = 100e6;
    ...
    if(minIndex < 2) {
        ...
        if(!CloseEnough(v.GetElement(2), 0.0)) {
            ...
            if(std::sqrt(std::pow(validPOI.GetElement(0), 2.0) + std::pow(validPOI.GetElement(1), 2.0)) < 1.0) {
                ...
                // Compute the local normal.
                qbVector<double> localOrigin{{0.0, 0.0, 0.0}};
                qbVector<double> normalVector{{0.0, 0.0, 0.0 + validPOI.GetElement(2)}};
                qbVector<double> globalOrigin = m_transformMatrix.Apply(localOrigin, qbRT::FWDTFORM);
