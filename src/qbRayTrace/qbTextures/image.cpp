...
bool qbRT::Texture::Image::LoadImage(const std::string &fileName) {
    if(m_imageLoaded)
        SDL_FreeSurface(m_imageSurface);
    ...
    if(!m_imageSurface) {
        QBERROR("Failed to load image. {}.", SDL_GetError());
        m_imageLoaded = false;
        return false;
    }
    ...
    QBINFO("Loaded {} by  {}.", m_imageSurface->w, m_imageSurface->h);
    QBINFO("Bytes per pixel = {}", m_bytesPerPixel);
    QBINFO("Pitch = {}", m_pitch);
