...
qbVector<double> qbRT::SimpleRefractive::ComputeTranslucency(const std::vector<std::shared_ptr<qbRT::ObjectBase>> &objectList,
                                                             ...
                                                             const qbVector<double> &localNormal, const qbRT::Ray &incidentRay) {
    qbVector<double> trnColor{3};
    ...
    qbVector<double> refractedVector = r * p +
                                       (r * c - std::sqrt(1.0 - std::pow(r, 2.0) * (1.0 - std::pow(c, 2.0)))) * tempNormal;
